...
2. Rolling upgrade with or without a deadline
   - They can have an associated deadline.
   - Deadline can be pushed if sufficient node operators have not upgraded (a slashable condition).
   - Rolling software upgrade also refer to mandatory upgrades which fix major node software issues. In this case the deadline is not set on-chain but node operators are asked to upgrades ASAP.
     
3. Height Coordinated Upgrade
   - Here height referes to the view height.
